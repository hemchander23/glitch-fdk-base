...
module.exports = function (self, options, tunnelURL) {

  // eslint-disable-next-line new-cap
  const genHookDefer = jQueryDeferred.Deferred();
  const eventsArray = _.map((self.svrExecScript.events || []), 'event');
  const isNotRegistered = !_.includes(eventsArray, EVENT_NAME);

  if (isNotRegistered) {
    genHookDefer.reject({ message: MISSING_EVENT_CALLBACK_ERR });
  }
  else if (!_.includes(EXTERNAL_EVENT_SUPPORT, self.event.categoryArgs.methodName)) {
