...
const errorHandler = require('./lib/utils/err');
const fileUtil = require('./lib/utils/file-util');
const help = require('./lib/cli/help');
const pjson = require('./package.json');
const updater = require('./lib/updater');
const writeMetric = require('./lib/utils/write-metric');
...
if (Number(process.versions.node.split('.')[0]) < NODE_MAJOR) {
  console.log('Node version of 8.x.x and above is required to run SDK. ' + process.versions.node + ' found.');
  process.exit(0);
}
// Cli Parsing:
...
function runCLI(error) {
  ...
  case 'run': {
    showHelp('run');
    require('./lib/cli/run').run(res.opts.has('c'), res.opts.has('s'), res.optArg.get('v'));
    break;
  }
